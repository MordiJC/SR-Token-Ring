...
{
  if(!instance) {
    instance = std::make_unique<QuitStatusObserver>();
  }
...
bool QuitStatusObserver::shouldQuit() const
{
  return shouldQuitStatus;
}

void QuitStatusObserver::registerHandler(QuitStatusObserver::QuitFunctionHandler_t handler)
{
  std::lock_guard<std::recursive_mutex> g(quitHandlersMutex);
  if(std::find(quitHandlers.begin(), quitHandlers.end(), handler) != quitHandlers.end()) {
    quitHandlers.push_back(handler);
  }
}

void QuitStatusObserver::unregister(QuitStatusObserver::QuitFunctionHandler_t handler)
{
  std::lock_guard<std::recursive_mutex> g(quitHandlersMutex);
